...
class StockAnalyzer:
    ...
    def __init__(self, initial_cash=1000000):
        ...
        self.gemini_model = os.getenv('GEMINI_API_MODEL', 'gemini-2.5-pro')
        self.function_call_model = os.getenv('FUNCTION_CALL_MODEL')
        ...
        try:
            ...
            tools = [{
                "function_declarations": [
                    {
                        "name": "search_news",
                        "description": "根据股票名称和代码搜索相关新闻、公告和行业动态",
                        "parameters": {
                            "type": "object",
                            "properties": {
                                "query": {
                                    "type": "string",
                                    "description": "用于搜索新闻的关键词，例如 '贵州茅台 600519 新闻'"
                                }
                            },
                            "required": ["query"]
                        }
            ...
            def call_api():
                try:
                    # 第一步：初始化模型，让它决定是否使用工具
                    # Gemini 的工具在模型初始化时定义
                    genai.configure(api_key=self.gemini_api_key)
                    model = genai.GenerativeModel(
                        model_name=self.function_call_model,
                        tools=tools
                    )
                    
                    # 使用 ChatSession 可以更方便地管理多轮对话历史
                    ...
                    if message.parts and message.parts[0].function_call:
                        # 处理工具调用
                        tool_call = message.parts[0].function_call
                        function_name = tool_call.name
                        
                        # Gemini 的 .args 直接就是字典，无需 json.loads
                        function_args = tool_call.args
                        
                        # 准备一个列表来存放工具的响应
                        tool_responses = []
                        
                        # 执行搜索 (这里假设只有一个工具调用，如果可能有多个，需要遍历)
                        if function_name == "search_news":
                            search_query = function_args.get("query", f"{stock_name} {stock_code} 新闻")
                            # 执行你的本地/外部函数
                            function_response_content = search_news(search_query)
                            
                            # 将工具的执行结果准备好，以便发回给模型
                            # 注意这里的结构，这是 Gemini API 要求的格式
                            tool_responses.append({
                                "tool_call": tool_call,
                                "tool_response": {
                                    "name": function_name,
                                    "content": json.dumps(function_response_content, ensure_ascii=False)
                                }
                            })

                        # 第二步：将工具的响应发回给模型，让它处理搜索结果并生成最终响应
                        # 在同一个 chat session 中发送工具的响应
                        second_response = chat.send_message(
                            # Gemini SDK v0.5.0 及以上版本简化了此步骤
                            # content 参数可以直接是 FunctionResponsePart
                            # 但为了兼容性，我们使用一个通用的字典结构
                            {
                                "role": "function",
                                "parts": [
                                    {
                        ...
                        # 将最终的文本响应放入队列
                        result_queue.put(second_response.text)
                    
                    else:
                        # 如果模型没有选择使用工具，直接使用第一次响应的文本
            ...
            try:
                ...
                # 提取回复内容
                # 修正点 2: `result` 现在是字符串，直接作为 content 使用
    ...
    def get_ai_analysis(self, df, stock_code, market_type='A'):
        ...
        try:
            ...
            # 设置API密钥和基础URL
            genai.api_key = self.gemini_api_key
            genai.api_base = self.gemini_api_url

            # 1. 获取最近K线数据
            # recent_data = df.tail(20).to_dict('records')
            # 保持为DataFrame格式，便于处理
            recent_k_data = df.tail(20) 

            def format_k_data_for_prompt(k_data_df):
                if k_data_df.empty:
                    return "无K线数据"
                
                # 创建一个简洁的字符串表示，而不是完整的字典
                # 格式: Date: C(收盘价) H(最高价) L(最低价) V(成交量) Change(涨跌幅)
                lines = []
                for _, row in k_data_df.iterrows():
                    # 计算涨跌幅
                    price_change_pct = (row['close'] - row['open']) / row['open'] * 100 if row['open'] != 0 else 0
                    change_symbol = "↑" if price_change_pct > 0 else "↓" if price_change_pct < 0 else "-"
                    ...
                    line = (
                        ...
                        f"H={row['high']:.2f} "
                        f"L={row['low']:.2f} "
                        f"V_Ratio={row['Volume_Ratio']:.2f} " # 使用量比更有意义
                        f"Chg={price_change_pct:.2f}% {change_symbol}"
                    )
                    lines.append(line)
                return "\n".join(lines)
            formatted_k_data = format_k_data_for_prompt(recent_k_data)
            ...
            prompt = f"""
# ROLE & GOAL
你是一名资深的量化与基本面结合的股票分析师。你的任务是根据下面提供的【输入数据】，为股票 {stock_name} ({stock_code}) 生成一份专业、全面、且具有高度可操作性的投资分析报告。你的分析必须严格基于提供的数据，并遵循【输出要求】中定义的格式。
...
### 2. 最近20日K线走势概要 (格式: 日期: C=收盘价 H=最高价 L=最低价 V_Ratio=量比 Chg=涨跌幅)
{formatted_k_data}

### 3. 核心量化指标摘要
- **短期趋势**: {technical_summary['trend']}
- **当前波动率**: {technical_summary['volatility']}
- **成交量趋势**: {technical_summary['volume_trend']}
- **RSI (14日)**: {technical_summary['rsi_level']:.2f}
- **MACD信号**: {technical_summary['macd_signal']}
- **布林带位置**: {technical_summary['bb_position']}

### 4. 关键支撑与压力位
- **短期支撑位**: {', '.join([str(level) for level in sr_levels['support_levels']['short_term']]) or 'N/A'}
- **中期支撑位**: {', '.join([str(level) for level in sr_levels['support_levels']['medium_term']]) or 'N/A'}
- **短期压力位**: {', '.join([str(level) for level in sr_levels['resistance_levels']['short_term']]) or 'N/A'}
- **中期压力位**: {', '.join([str(level) for level in sr_levels['resistance_levels']['medium_term']]) or 'N/A'}

### 5. 量化评分体系详解 (总分: {score_details['total']})
{self._format_score_breakdown_for_prompt(getattr(self, 'score_breakdown', None))}

### 6. 综合投资建议
- **初步建议**: {recommendation}

### 7. 市场与新闻信息
- **市场情绪**: {news_data.get('market_sentiment', 'neutral')}
- **近期相关新闻**:
...
{'### 8. 原始新闻内容 (JSON解析失败时的参考)' if not self.json_match_flag else ''}
{news_data.get('original_content', '') if not self.json_match_flag else ''}

---
# 【输出要求】

请严格按照以下Markdown格式生成报告，确保所有标题和结构都完全一致。

# 股票分析报告：{stock_name} ({stock_code})

## 1. 综合评估与核心观点
- **一句话总结**: (用一句话概括当前股票的核心状态和投资价值)
- **量化总评**: (基于总分 {score_details['total']} 分，解读其在市场中的相对位置和机会等级)
- **核心优势**: (列出1-3个当前最主要的积极因素，如“趋势强劲”、“资金介入明显”等)
- **主要风险**: (列出1-3个当前最需要警惕的风险点，如“RSI超买”、“面临关键压力位”等)

## 2. 技术面深度分析
- **趋势与动量**: 结合【量化评分体系详解】中的趋势和动量评分，详细分析当前股价所处的长、中、短期趋势状态。解读MA均线排列的含义。
- **关键指标解读**: 深入解读RSI、MACD和布林带指标。RSI({technical_summary['rsi_level']:.2f})是否处于超买/超卖区？MACD金叉/死叉的强度如何？价格在布林带中的位置({technical_summary['bb_position']})预示着什么？
- **量价关系分析**: 分析成交量 ({technical_summary['volume_trend']}) 与价格走势的配合情况。是放量上涨、缩量回调还是价量背离？这反映了什么市场意图？
- **支撑与压力**: 明确指出当前价格上方面临的主要压力位和下方的关键支撑位，并解释其重要性。

## 3. 行业与市场环境分析
- **行业动态**: 结合【行业动态】新闻，分析该股所处行业景气度及近期变化。
- **市场情绪**: 结合【市场情绪】和【近期相关新闻】，分析当前围绕该股的市场氛围是偏乐观还是悲观，以及可能的原因。
- **公司动态**: 从【公司公告】中提炼出可能对股价产生重大影响的事件（如业绩预告、分红、重大合同等）。

## 4. 风险因素评估
- **技术面风险**: 详细说明技术指标发出的警示信号（例如：顶背离、超买严重、跌破关键支撑等）。
- **消息面风险**: 分析新闻或公告中可能存在的潜在利空因素。
- **市场风险**: 评估宏观市场或板块轮动可能对该股造成的不利影响。

## 5. 具体交易策略
### 5.1 主要策略 (基于当前分析)
- **操作建议**: {recommendation}
- **理想入场点位区间**: (给出一个具体的价格范围)
- **止损位**: (设置一个明确的止损价格)
- **短期目标价位 (1-4周)**: (设置一个现实的短期目标价)
- **中期目标价位 (1-3月)**: (设置一个中期目标价)

### 5.2 备选策略 (情景分析)
- **若股价强势突破短期压力位**: (可以采取什么追涨或加仓策略？)
- **若股价意外跌破短期支撑位**: (应该如何应对？是离场观望还是等待更低位置？)
...
请保持客观、中立的分析师口吻，分析内容需紧扣数据，避免使用模糊和主观性过强的词汇。
"""
            # ================================= PROMPT END =================================
            # 修正点 1: 使用生成的 prompt
            # 原openai
            ...
            def call_api():
                try:
                    # 修正点 1: 正确初始化 Gemini 模型
                    genai.configure(api_key=self.gemini_api_key)
                    model = genai.GenerativeModel(
                        model_name=self.gemini_model,
                        # 修正点 2: 将参数放入 generation_config
                        generation_config={
                            "temperature": 0.8,
                    ...
                    # 修正点 3: 使用正确的 API 调用方法 generate_content
                    # 对于单轮对话，直接传入 prompt 字符串即可
                    response = model.generate_content(prompt)
                    
                    # 修正点 4: 将提取出的文本放入队列
                    # Gemini 的响应可以直接通过 .text 属性获取
                    result_queue.put(response.text)
    ...
    def _format_score_breakdown_for_prompt(self, score_breakdown):
        """将详细的评分分解格式化为prompt字符串"""
        if not score_breakdown:
            return "   无详细评分数据"

        formatted = ""
        for key, value in score_breakdown.items():
            formatted += f"    - **{key.capitalize()}评分: {value.get('score', 0)} / {value.get('max_score', 0)} (权重: {value.get('weight', 0)}%)**\n"
            formatted += f"      - 描述: {value.get('description', '')}\n"
            if 'logic' in value:
                for logic_item in value['logic']:
                    formatted += f"      - 细节: {logic_item}\n"
            if 'indicators' in value:
                indicators_str = ", ".join([f"{k}: {v:.2f}" for k, v in value['indicators'].items()])
                formatted += f"      - 依据指标: {indicators_str}\n"
        return formatted
